...
export default function EditListeningExercise({
  session,
}: {
  session: Session;
}) {
  ...
  useEffect(() => {
    console.log(
      "EDIT_LISTENING_EXERCISE_" + local.exercise_id + " page loaded."
    );
    displayDetails();
  }, []);
  ...
  const [exercise, setExercise] = useState<ListeningExercise>({
  ...
  const save = async () => {
    if (exercise) {
      setDisabled(true);
      setLoading(true);
      await ExerciseService.updateListeningExercise(exercise);
      setLoading(false);
      setSaved(true);
    }
  };
        ...
        <ScrollView>
          ...
          >
            <Text fontSize={20} fontWeight={900} color={"$color"}>
              Edit Listening Exercise No. {local.exercise_id as string}
            </Text>
            ...
            <Fieldset horizontal ai="stretch">
              ...
              >
                ...
                <Text
                  ...
                  fontSize={"$5"}
                  onPress={() => {
                    let updated_exercise: ListeningExercise = { ...exercise };
            ...
            <XStack alignSelf="flex-end" gap="$4">
              <Link href="/exercises/listening" asChild disabled={saved}>
                <Button
                  theme="active"
                  aria-label="Close"
                  onPress={save}
