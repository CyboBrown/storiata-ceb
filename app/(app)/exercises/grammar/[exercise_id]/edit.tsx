...
export default function EditGrammarExercise({ session }: { session: Session }) {
  ...
  useEffect(() => {
    console.log("EDIT_GRAMMAR_EXERCISE_" + local.exercise_id + " page loaded.");
    displayDetails();
  }, []);
  ...
  const save = async () => {
    if (exercise) {
      setDisabled(true);
      setLoading(true);
      await ExerciseService.updateGrammarExercise(exercise);
      setLoading(false);
      setSaved(true);
    }
  };
        ...
        <ScrollView>
          ...
          >
            <Text fontSize={20} fontWeight={900} color={"$color"}>
              Edit Grammar Exercise No. {local.exercise_id as string}
            </Text>
